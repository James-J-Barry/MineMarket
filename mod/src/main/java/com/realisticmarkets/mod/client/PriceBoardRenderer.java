...
/**
 * Draws a Price Board's four rows: item icon, then "Bid $x ▲" and "Ask $y". The arrow compares the market price
 * with Normal (above by 2%: ▲, below: ▼). Text is full-bright so the board reads like a lit chalkboard.
 */
public class PriceBoardRenderer implements BlockEntityRenderer<PriceBoardBlockEntity, PriceBoardRenderer.State> {
    private static final int FULL_BRIGHT = 0xF000F0;
    private static final float ROW = 0.1875f;          // board is 12 px tall = 0.75 blocks, four rows
    ...
    public void submit(State state, PoseStack pose, SubmitNodeCollector collector, CameraRenderState camera) {
        ...
        for (int i = 0; i < state.count; i++) {
            ...
            pose.pushPose();
            pose.translate(-0.5f + 0.11f, rowTop - ROW / 2 + 0.01f, FRONT + 0.01f);
            pose.scale(0.15f, 0.15f, 0.01f);
            state.items[i].submit(pose, collector, state.lightCoords, OverlayTexture.NO_OVERLAY, 0);
            pose.popPose();

            pose.pushPose();
            pose.translate(-0.5f + 0.22f, rowTop - 0.015f, FRONT);
